...
class CompletionHandler:
    ...
    def __init__(self, element_handler, matching_frames):
        ...
        self.video_name = element_handler.selected_file_path.split("/")[-1].split(".")[
            0
        ]
    ...
    def _setup_pdf(self, report_path):
        ...
        pdf_canvas = canvas.Canvas(
            f"{report_path}/completion_report_{self.video_name}.pdf", pagesize=A4
        )
    ...
    def _generate_pdf_content(self, pdf_canvas):
        ...
        for i, frame in enumerate(self.matching_frames):
            ...
            pdf_canvas.drawCentredString(
                A4[0] / 2, A4[1] - 50, f"Report for {self.video_name}"
            )
            ...
            pdf_canvas.drawString(
                50, y_position, f"Matching Frame: {frame['filename'].split('/')[-1]}"
            )
            y_position -= 20
            ...
            for word in words:
                if (
                    len(current_line + " " + word) <= max_chars_per_line
                    or not current_line
                ):
            ...
            pdf_canvas.rect(
                table_x,
                table_y - caption_row_height - table_row_height,
                table_width,
                table_row_height + caption_row_height,
            )
            pdf_canvas.line(
                table_x + table_width * 0.5,
                table_y - caption_row_height - table_row_height,
                table_x + table_width * 0.5,
                table_y,
            )
            pdf_canvas.line(
                table_x + table_width * 0.75,
                table_y - caption_row_height - table_row_height,
                table_x + table_width * 0.75,
                table_y,
            )
            ...
            pdf_canvas.drawCentredString(
                table_x + table_width * 0.25, table_y - 15, "Caption"
            )
            pdf_canvas.drawCentredString(
                table_x + table_width * 0.625, table_y - 15, "Timestamp"
            )
            pdf_canvas.drawCentredString(
                table_x + table_width * 0.875, table_y - 15, "Frame Number"
            )
            ...
            caption_left_margin = (
                table_x + 10
            )  # 10 pixels from the left border of the cell
            ...
            pdf_canvas.drawCentredString(
                table_x + table_width * 0.625,
                table_y - table_row_height - (caption_row_height / 2),
                timestamp_formatted,
            )
            pdf_canvas.drawCentredString(
                table_x + table_width * 0.875,
                table_y - table_row_height - (caption_row_height / 2),
                str(frame["filename"].split("/")[-1].split("_")[0]),
            )
